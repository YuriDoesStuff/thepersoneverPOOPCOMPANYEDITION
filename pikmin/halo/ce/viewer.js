...
fetch(`a/`+ params.get('log') +`.json`, {cache: "no-cache"})
 ...
 maindiv.innerHTML = `<h1>ughhh</h1>
 <p>um. url didn't have enough queries!<p>
 <a href="?a=mission1&pg=1">go to page one of mission one?</a>
 `;
}
...
searchnum = searchnum.toString();
 ...
 switch(pages[searchnum].type){
   case("img"):
    image.innerHTML = "<img src="+glojson.imgdir+pages[searchnum].url+">";
    break;
   case("swf"):
     image.innerHTML = `<div class=flash><div id=swf></div></div>`;
     div = document.getElementById("swf");
     swfobject.embedSWF(glojson.imgdir+pages[searchnum].url, div, "100%", "100%", 10);
  }
